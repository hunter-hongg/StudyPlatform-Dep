...
pub const GuWanChaFiles = extern struct {
    ChaHu: [*:0]u8,
    ChaZhan: [*:0]u8,
    AddNum: c_int,
};

export fn GuWanChaFiles_Make(chahu: [*:0]u8, chazhan: [*:0]u8, addnum: c_int) *GuWanChaFiles {
    const tmp = callocator.create(GuWanChaFiles) catch unreachable;
...
export fn GuWanChaFiles_Free(file: *GuWanChaFiles) void {
    callocator.free(std.mem.span(file.ChaHu));
    callocator.free(std.mem.span(file.ChaZhan));
    callocator.destroy(file);
}
export fn GuWanChaFiles_Read(file: *GuWanChaFiles, flag: c_int) c_int {
...
export fn GuWanChaFiles_Add(filea: *GuWanChaFiles, flag: c_int, addnum: c_int) void {
    const file = if(flag == 0)
        std.mem.span(filea.ChaHu)
    else 
        std.mem.span(filea.ChaZhan);
    const tmp = GuWanChaFiles_Read(filea, flag);
